...
public class AndroidFASTSettings implements FASTSettings {
    ...
    //TODO: reword
    /**
     * Used to obtain a sequence number to be used with pm.getChangedPackages()
     * Checks whether the device was rebooted since the last app start
     * (technically, the last call of this method), initializes the knownSequenceNumber
     * and updates the knownBootCount. If no initialisation is needed, the previously stored
     *
     * @return Either a sequence number previously stored using putSequenceNumber(int sequenceNumber)
     *          or a freshly initialized as -1 indicating that a reboot was there
     */
    @TargetApi(26)
